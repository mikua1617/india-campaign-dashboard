"""
Lead-level engagement sync -> one persistent Google Sheet per active India campaign.

What it does, each run:
  1. For every active India_ campaign, pulls every lead via /leads (paginated) --
...
SHARED_DRIVE_FOLDER_ID = "1FX3DY7-lLd1ZH5BYe_oDOazAk3WKxxno"  # "India" subfolder inside the shared drive
...
def get_or_create_master_pivot_sheet(links):
    key = "_master_pivot"
    if key not in links:
        sheet_id = create_sheet_for_campaign("India Campaigns — Master Engagement Pivot")
        links[key] = {
            "sheet_id": sheet_id,
